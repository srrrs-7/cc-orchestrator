...
# app/auth・app/iac・app/migrator)/ bin/bun(app/web)側にあり、ここは
# `$(MAKE) -C ...` / `bin/bun run ...` を呼ぶだけ。コマンド名の正は
...
# パス引数を渡したい場合は従来どおり `cd app/web && bun run test <path>` を使う。
# ---------------------------------------------------------------------------
api-%:
	$(MAKE) -C app/api $*
