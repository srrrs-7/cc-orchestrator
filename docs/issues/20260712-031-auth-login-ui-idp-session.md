...
- [ ] Spec 起票または SPEC-015 後継 Spec でログインフロー要件を確定
- [ ] ISSUE-005 解消(パスワードハッシュ + 定数時間比較)
- [ ] `/authorize` 未ログイン時の login redirect / 302 ではなく login 画面表示
- [ ] ログイン成功後の authorize 再開(state/PKCE 保持)
- [ ] IdP セッション TTL / ログアウト(ISSUE-033)との整合
- [ ] route 統合テスト + review-security
