...
- [x] `GET /logout` ハンドラ(end_session)
- [x] Discovery `end_session_endpoint` 公開
- [x] `post_logout_redirect_uri` / `state` / `client_id` 検証
- [x] web Sign out フロー更新
- [x] テスト(`logout_flow_test.go`)
...
- `GET /logout` + Discovery `end_session_endpoint`。web `logout()` が end_session へ redirect。`make -C app/auth check` / `make -C app/web check` green。
