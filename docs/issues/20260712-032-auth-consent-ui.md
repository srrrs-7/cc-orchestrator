...
- [x] consent 記録スキーマ(goose) + repository
- [x] consent 画面(HTML)
- [x] Accept / Deny 分岐と OAuth エラーマッピング(`access_denied`)
- [x] 再 authorize 時のスキップ条件
- [x] テスト(`consent_flow_test.go`)
...
- `consents` テーブル + ConsentService + GET/POST `/consent`。`make -C app/auth check` green。status を `resolved` に更新。
