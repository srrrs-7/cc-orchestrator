...
- [x] spec-owner / 次サイクルで方針(a / b)を確定する → 方針 (b) を採用
- [x] (b) の場合: config から `@tanstack/react-query` を除去し再生成、Spec §4 の記述を再整合(impl-web / spec-owner)
...
- 方針 **(b)「使わない前提を明確化する」** で解消した。
- `openapi-ts.config.ts` の `plugins` から `"@tanstack/react-query"` を除去し、代わりに `"@hey-api/sdk"` を明示追加(next プレリリース版では SDK 関数生成プラグインを明示しないと `sdk.gen.ts` が出力されなくなるため)。
- `make -C app/web generate` 再実行: `@tanstack/react-query.gen.ts`(dead code 217 行)および `@tanstack/` ディレクトリが削除された。`sdk.gen.ts` / `index.ts` は従来と等価な状態を維持。
- `make -C app/web check` 全通過(format-check / lint / typecheck / test 140 件 / build)。hooks(`useTasks.ts`)は `@tanstack/react-query` パッケージの `useQuery` / `useMutation` を直接利用しており、削除対象だった generated ファイルには依存していないため影響ゼロ。
- SPEC-003 §4「hooks は生成 query オプションを利用する」記述との整合は spec-owner 判断事項として残るが、dead code / ドリフト検査対象の除去は完了。status を **resolved** へ変更。
</content>
</invoke>
