...
- [x] SPEC-003 の D2 ゲートで契約統一方針(a / b / c)を確定する(planner + spec-owner + ユーザー判断)→ **(a)「web を Go の start/complete に合わせる」で確定**(SPEC-003 経緯 2026-07-08 の D2/D3 判断。backend は変更しない)
- [x] 確定方針に沿って web / api の遷移契約を一致させる(impl-web / impl-api)→ **web を `POST /tasks/{id}/start|complete` に移行**(`client.ts` / `useTasks.ts` / `TaskItem.tsx` / `mocks/handlers.ts`)。api は元から Go 契約で変更なし
- [x] 遷移契約のテストを両 stack で整合させる(tester)→ web 側テストを移行(`client.test.ts:72` / `useTasks.test.tsx:45` が `updateTaskStatus` / `useUpdateTaskStatus` の消滅を検証、`TaskItem.test.tsx:71,98` が start/complete 呼び出しを検証)。api の start/complete テストは既存を維持
- [x] SPEC-003 R6 のドリフト検査で再発を機械的に検出できる状態にする → `.github/workflows/contract-drift.yml` を追加(Go→openapi.yaml→web 生成物の再生成 + `git diff --exit-code`)
...
- **SPEC-003 の D2 解消により解決。** D2 ゲートはユーザー判断で **(a)「web を Go の `POST /tasks/{id}/start|complete` に合わせる」**(本 Issue 対応方針の候補 a、SPEC-003 経緯 2026-07-08 の「D2・D3 = web を Go に合わせる」)で確定し、SPEC-003 の実装で web を Go 契約へ移行した。backend(app/api)は元から Go 契約のため変更なし。
- 検証方法: web 実コードとテストのコード観察(D2 の解消は「web の遷移リクエストが api の実エンドポイントに一致すること」なので、web 側の契約が Go(`POST …/start`・`…/complete`・body 無し)に一致したことを確認すれば足りる)。
- 検証結果(事実):
  - `app/web/src/features/tasks/api/client.ts:106-116` — `startTask`(`POST /tasks/:id/start`)/ `completeTask`(`POST /tasks/:id/complete`)に移行。コメントで「D2: replaces the old `PATCH /tasks/:id/status` call; Go's wire contract has no PATCH .../status endpoint」と明記。旧 `updateTaskStatus` は撤去(`client.test.ts:72` が「`updateTaskStatus` no longer exists」を検証)。
  - `app/web/src/features/tasks/hooks/useTasks.ts` — `useStartTask` / `useCompleteTask` の 2 mutation へ移行(旧 `useUpdateTaskStatus` は撤去。`useTasks.test.tsx:45` が消滅を検証)。
  - `app/web/src/features/tasks/components/TaskItem.tsx:5,20-30` — `useStartTask` / `useCompleteTask` を利用し、クリックしたボタンで start / complete を呼び分ける(遷移種別を URL で表現)。`TaskItem.test.tsx:71,98` が「POST .../start・.../complete 呼び出し(PATCH .../status + body ではない)」を検証。
  - `app/web/src/mocks/handlers.ts:119,123` — MSW を `http.post("/api/tasks/:id/start")` / `http.post("/api/tasks/:id/complete")` に移行(旧 `http.patch(.../status)` は撤去、エラーは `{error}` を返す形へ)。
  - api 側は変更なし(`POST /tasks/{id}/start|complete`、body 無し、`{error}` エラー包み)。web が api の実契約に一致し、乖離の 3 軸(メソッド / パス粒度 / ボディ)がすべて解消した。
- 再発防止(導入済み): SPEC-003 R6 のドリフト検査 `.github/workflows/contract-drift.yml` が、Go→`app/api/docs/openapi.yaml`→web 生成物の再生成 + `git diff --exit-code` で契約ズレを CI で機械検出する。今後の遷移契約ズレはこのジョブで落ちる。
- 残る `PATCH /tasks/:id/status` の文字列参照は、いずれも撤去を説明するコメント / テスト(`handlers.ts:27`、`useTasks.ts:41,55`、`client.ts:107-108`、各 `.test`)に限られ、実 import / 実リクエスト経路には存在しない(`grep` で確認)。
- status を `open` → **`resolved`** に更新した。判定根拠: D2 の解消(web を Go 契約へ寄せる)が web の実コードとテストで検証でき、乖離が消え、R6 ドリフト検査で再発が機械検出される状態になったため。
- 関連: SPEC-003 側の §3 R1 endpoint 記述は既に Go 実体(`POST …/start`・`…/complete`)へ訂正済み(SPEC-003 経緯 2026-07-08)。相互リンク(frontmatter `specs: [SPEC-003]` / SPEC-003 側 `issues`)は維持。
</content>
</invoke>
