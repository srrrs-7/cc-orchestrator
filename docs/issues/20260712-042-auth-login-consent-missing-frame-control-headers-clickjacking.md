...
- 修正(impl-auth + impl-iac): app/auth に `route/security_headers.go` を新設し、`securityHeaders` ミドルウェアを `router.go` の最外周(admin 経路含む全ルート)に配線。全レスポンスに `X-Frame-Options: DENY` / `Content-Security-Policy: default-src 'none'; style-src 'self' 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'` / `X-Content-Type-Options: nosniff` / `Referrer-Policy: strict-origin-when-cross-origin` を付与するようにした。app/iac は `modules/cdn/main.tf` に `auth_security` response_headers_policy(frame_options DENY / nosniff / referrer / HSTS、CSP は app 側所有のため付けない)を新設し `/auth/*` behavior に適用(多層防御)。
- 検証: tester が実 test DB で全 180 テスト PASS・回帰なし(ミドルウェアは既存ヘッダ assert と無衝突)+ JSON discovery / consent 実描画 / token 成功のカバレッジテスト 3 件を追加。review-security が「全ルート・エラー / リダイレクト / admin 経路を必ず通過」「CSP が login/consent の実描画を壊さず frame-ancestors 'none' でクリックジャッキングを防ぐのに十分」「CDN ポリシーとアプリ CSP が非衝突」を確認した。Blocker 解消につき resolved。
- フォローアップ(今回対応外・任意): 再レビューで `/auth/*` の HSTS に `preload` 未付与という Minor 強化余地が指摘された(悪用シナリオではなく強化)。必要なら別途対応。
