...
- **closed(許容・上流待ち)**。`@hey-api/openapi-ts` は TS7 対応の `0.0.0-next-*` prerelease にピン(SPEC-007 由来、`openapi-ts.config.ts` コメント参照)。`@hey-api/client-fetch` プラグインは引き続き `createSseClient` を `client.gen.ts` に無条件同梱(SSE 本体 ~242 行)。本 API に SSE エンドポイントは無く、推定 gzip 数 KB のバンドル無駄のみ。
- 現行 prerelease / stable 0.99.x いずれも SSE 除外 config オプションは未提供(設定ミスではなく生成物構造)。実害はサンプル規模で negligible。
- stable TS7 対応版 + SSE 除外が提供された時点で再オープンまたは新 Issue で追随。現時点では追跡目的を達成し close。
</content>
