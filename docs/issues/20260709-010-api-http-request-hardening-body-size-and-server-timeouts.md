...
- [x] ボディサイズ上限(`http.MaxBytesReader`)を全デコード経路に適用する方式と上限値を決めて実装する(2026-07-12 実装済み。`decodeJSONBody` ヘルパを `app/api/route/response.go` に追加、上限 1 MiB)
- [x] `http.Server` に `ReadTimeout` / `ReadHeaderTimeout` / `WriteTimeout` / `IdleTimeout` を設定する(2026-07-10 実装済み。ISSUE-024 の gosec G112 対応で app/auth と対称化。`MaxHeaderBytes` は未設定で残課題)
- [x] 上限超過を 413 相当で扱う(2026-07-12 実装済み。`*http.MaxBytesError` を `errors.As` で検出し 413 を返す)
- [x] テスト追加(上限超過時のステータス/エラー)(2026-07-12 実装済み。`TestOversizedBody_Returns413` で create / changePriority の両エンドポイントを検証)
...
### 2026-07-10(gosec 統合〈ISSUE-024〉でタイムアウト部分〈G112〉を解消 / body-size 上限は未対応・open 維持)

- ISSUE-024(gosec を Go 3 スタックの lint / CI に恒久組み込み)の対応の一環として、本 Issue のうち **サーバタイムアウト部分(gosec G112 Potential Slowloris Attack)を解消**した。CI pin の golangci-lint 1.64.8 の gosec による baseline 実測で app/api に G112 が 1 件検出されたため、`app/api/cmd/api/main.go` の `http.Server` に `ReadHeaderTimeout`(5s)/ `ReadTimeout`(10s)/ `WriteTimeout`(10s)/ `IdleTimeout`(60s)を実装(app/auth 側 `cmd/authz/main.go` と対称化。`newServer` を抽出して配線)。tester が `main_test.go` に回帰テストを追加し、再スキャンで gosec G112 は 0 件、checker が `make check` green を 1.64.8 で確認。
- **本 Issue は open 維持**。理由: 本 Issue はサーバタイムアウトと **リクエストボディサイズ上限**(`http.MaxBytesReader` 等・413 相当の扱い)の 2 本柱で、今回解消したのはタイムアウト側のみ。body-size 上限は **gosec では検出されない**課題(静的解析の対象外)であり ISSUE-024 のスコープ外のため未対応で残る(別途対応。「実施内容」の body-size 関連チェックは未消化)。`MaxHeaderBytes` も今回は未設定で残課題。
- 補足(検出ツールの制約): 今回の検出・回帰検証は CI pin の gosec **1.64.8** による。G112 はこのバージョンでも検出可能なため機械回帰検出が効く(SSRF/G704・open-redirect/G710 のように golangci-lint v2 系を要するルールとは異なる)。詳細は ISSUE-024 参照。
- ステータスは `open` のまま。`updated` は 2026-07-10。

### 2026-07-12(ボディサイズ上限 / 413 対応を実装・解消)

- 本 Issue のうち残存していた「ボディサイズ上限(`http.MaxBytesReader`)+ 413 返却」部分を実装し、Issue を **resolved** にクローズした。
- **実装内容**: `app/api/route/response.go` に `decodeJSONBody` ヘルパを追加。`http.MaxBytesReader(w, r.Body, 1<<20)` で上限を 1 MiB に設定し、`json.Decoder` でデコード。エラー時は `errors.As(err, &*http.MaxBytesError)` で分岐し、上限超過は 413(Request Entity Too Large)、その他の decode 失敗は従来通り 400(Bad Request)を返す。`create`・`changePriority` ハンドラの `json.NewDecoder(r.Body).Decode` を `decodeJSONBody` 呼び出しに置き換えた。未使用になった `encoding/json` import を `task_handler.go` から削除。
- **テスト**: `task_handler_test.go` に `TestOversizedBody_Returns413` を追加。1 MiB + 1 バイトのボディで `POST /tasks` と `POST /tasks/{id}/priority` の両エンドポイントが 413 を返すことを検証。
- **確認**: `make check`(toolchain コンテナ内)green。lint 0 issues / vet pass / build pass / test pass(route パッケージ含む全テスト)。
- commit: `fix(api): add request body size limit via MaxBytesReader, return 413 on overflow (ISSUE-010)` (branch: feat/auth-oidc-foundation)
- **残課題**: `MaxHeaderBytes` は今回も未設定(http.Server デフォルト 1 MiB のため緊急度なし。別 Issue として必要であれば起票)。
