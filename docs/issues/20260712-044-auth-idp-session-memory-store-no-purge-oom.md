...
- 修正(impl-auth): `infra/memory/idp_session_store.go` に `PurgeExpired(ctx) (int64, error)` を追加(sessions / pending 両 map を単一 Mutex 保護下で `now.After(ExpiresAt)` 判定で一括削除)。`cmd/authz/main.go` の既存 `runPurgeTicker`(authcode / refreshtoken 用・15 分間隔)に第 3 引数として相乗り配線した。
- 検証: tester の単体テスト(期限切れのみ削除・生存温存・no-op・ctx キャンセル)PASS。review-security が「生存エントリを誤削除しない」「map の並行アクセスは Mutex で保護・race 無し」「ticker で確実に定期実行される」を確認した。Major 解消につき resolved(最小の in-memory purge。Postgres 化の恒久策は要否検討のまま残置)。
