...
// that were previously duplicated inside infra/postgres/*_test.go
// files, and makes them available as an importable package so that
// route tests can reuse the same connection and truncation logic
...
// against a dedicated test database (DB_NAME defaults to "api_test",
// never the "api" database used by `make up`). There is no longer an
...
//
// DB_NAME defaults to "api_test" (a dedicated test database, separate
// from the "api" database used by `make up`/local development, so
...
// persistence. DB_NAME defaults to "api_test".
