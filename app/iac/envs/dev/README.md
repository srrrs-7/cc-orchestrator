...
`ISSUER` 環境変数(`https://<cloudfront-domain>/auth`。ISSUE-043 で `http://` から修正済み
-- CloudFront は全 behavior で `redirect-to-https` を強制するため実アクセスは常に HTTPS であり、
scheme を http のままにすると app/auth の `SecureCookiesFromIssuer` が `idp_session` /
`idp_pending` Cookie に `Secure` を付けない)は `module.cdn.cloudfront_domain_name` に依存し、
`module.cdn` は `module.platform.alb_dns_name` にのみ依存する(ECS サービスの登録状態には
依存しない)。そのため `platform → cdn → service_auth` の一方向 DAG が成立し、初回 `apply`
一発で auth タスクに実際の CloudFront ドメインを注入できる(`module.app` を経由した循環に
ならない設計。詳細は `modules/platform/README.md`)。auth は プロセス起動ごとに RSA 署名鍵を
生成するため(`app/auth` の既知の性質)、`desired_count = 1` を既定にしている
(`modules/service/README.md` 参照)。

同じ `module.cdn.cloudfront_domain_name` から `main.tf` の共有 `local.auth_*` ブロックが
`local.auth_audience`(`https://<cloudfront-domain>/api`)・`local.auth_jwks_url`
(`${local.auth_issuer}/.well-known/jwks.json`)も導出し、`module.service_auth` の
`ISSUER`/`API_AUDIENCE` と `module.service_api` の `AUTH_ISSUER`/`AUTH_JWKS_URL`/
`AUTH_AUDIENCE` の両方に配線する(ISSUE-041)。これらを個別の `variable` にしなかったのは、
`module.cdn` の出力から機械的に導出することで、tfvars への設定漏れという失敗モード自体を
発生させないため(値を書き忘れて未認証公開に戻る、という ISSUE-041 の再発を構造的に防ぐ)。
