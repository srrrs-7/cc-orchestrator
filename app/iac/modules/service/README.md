...
goose 自身のバージョン管理表(`goose_db_version`)は、対象スキーマ(`DB_SCHEMA`、api=`api` /
auth=`auth`)の中に作られる。つまり **goose を実行する前にそのスキーマ自体が存在している必要が
ある**(鶏卵問題)。この作成を Terraform 側(例: `postgresql` プロバイダで `postgresql_schema`
リソースを作る)で行う案は、RDS が `publicly_accessible = false` の private subnet 内にあり、
Terraform を実行する場所(CI ランナー・開発者端末)から直接到達できないため採らなかった
(VPN/踏み台が別途必要になり運用コストが高い)。同様に、goose のマイグレーションファイル自体に
`CREATE SCHEMA` を書く案も、そのマイグレーション自身が適用される前提であるバージョン管理表が
そのスキーマの中にまだ無いという同じ鶏卵問題に当たる。

採用した方式は **migrate イメージのエントリポイント(`app/{api,auth}/Dockerfile.migrate`、
impl-db 管轄)が `goose up` を呼ぶ前に `CREATE SCHEMA IF NOT EXISTS "$DB_SCHEMA"` を実行する**、
というもの。この migrate コンテナは(a) ECS タスクの中(= RDS と同じ VPC 内)で実行されるため
到達性の問題が無く、(b) goose の外側(ラッパースクリプト)で行われるため鶏卵問題も起きない。
`envs/dev` はこのコンテナに渡す `DB_*` 環境変数(接続先・スキーマ名)と secrets(master 資格情報の
JSON key)を配線するところまでを担い、実際のスキーマ作成ロジックはイメージ側の責務として本
モジュールは関知しない。

### イメージ・並行実行・代替案

- `var.migration_image` は既定(空文字列)で `container_image` と同じパターンにより
  この service インスタンス自身の ECR リポジトリの `:migrate` タグを指す。`:latest`(アプリ本体)
  とは別タグの、`Dockerfile.migrate` から別途ビルドしたイメージを push する想定(SPEC-005
  plan §6.2 R-h。**このイメージの push 経路(CI/Makefile 拡張)は本 Spec の範囲外として後続に
  委ねられている**。`envs/dev` にこのコンテナを配線した時点では `:migrate` タグはまだ存在しない
  ため、そのまま `apply` すると新しいデプロイのタスクがイメージ pull に失敗して起動できず、
  ロールアウトが詰まる(既存タスクは running のまま残る)。**`:migrate` イメージを push してから
  apply すること**
- 並行実行の競合(SPEC-005 plan §6.2 R-g): 同一サービスで `desired_count` を 2 以上にすると、
  ローリングデプロイの瞬間に複数の新タスクが同時に起動し、それぞれの migrate コンテナが同じ
  スキーマに対して `CREATE SCHEMA IF NOT EXISTS` / `goose up` を並行実行し得る。`envs/dev` の
  既定 `desired_count = 1`(auth は JWKS 単一化の制約からも 1 固定)ではこの競合は発生しない。
  `desired_count` を増やす場合は、goose 側のアドバイザリロック(migrate イメージのエントリ
  ポイント側で有効化するかどうかは impl-db の責務)か、下記の代替案への切り替えを検討すること。
  api の migrate コンテナは `api` スキーマのみ、auth の migrate コンテナは `auth` スキーマのみを
  触るため、api と auth の migrate コンテナ同士が競合することは無い
- 代替案として **一回限りの ECS タスク**(`aws_ecs_service` を持たない `aws_ecs_task_definition`
  ...
  ここに記録するに留める(SPEC-005 plan §1.2 の採用理由・§6.2 R-g 参照)
