...
//
// cmd/authz/env.go additionally reads DB_READER_HOST/PORT/NAME/USER/
// PASSWORD/SSLMODE into Env.DBReader (docs/plans/SPEC-010-plan.md's
// "変更ファイル" table for cmd/authz/env.go, symmetric to cmd/api's),
// falling each unset item back to the corresponding writer DB_* value
// inside NewEnv itself -- so by the time NewEnv returns, Env.DBReader
// already holds the *effective* reader values, not the raw (possibly
