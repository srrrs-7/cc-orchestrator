...
| stack | 実行場所 | ツール |
|---|---|---|
| web | `app/web` | make(実体は `app/web/Makefile`)— `make check`(= format-check + lint + typecheck + test + build)、個別に `make format-check` / `format` / `lint` / `typecheck` / `test` / `build` / `generate` / `install`。単一テストは `make test TEST=<path>` |
