...
- **`//go:build integration`(実 DB、`make test-integration` に載る)**: 正常系・全フロー。`infra/postgres/testsupport` の `OpenTestDB`/`Truncate...`/seed ヘルパで実 DB に接続し、実 `postgres.New*Repository` を配線する
