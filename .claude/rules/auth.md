...
`crypto/*`・`encoding/*`、refresh token は `crypto/rand` / `crypto/sha256`)。**永続化層
...
- `domain/<aggregate>/` — ドメイン層(他層に非依存)。集約ごとにパッケージを切る(現状 `client` / `user` / `authcode` / `refreshtoken` の 4 集約 +(JWT 署名ポートのみの)`token`)
